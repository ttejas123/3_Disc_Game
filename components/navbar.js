...
function Navbar({tab, setTab, setGallery}) {
  return (
    <div className={`bg-]-100 p-2 mx-5 my-5 rounded-lg flex justify-between shadow-lg`}>
        ...
        <div className='flex items-center justify-between'>
            ...
            <div className='px-4 font-semibold text-sm text-pink-400 hover:text-purple-500 cursor-pointer' onClick={()=> setTab(1)} >CREATE METADATA</div>
            <div className='px-4 font-semibold text-sm text-pink-400 hover:text-purple-500 cursor-pointer' onClick={()=> setTab(2)} >CREATE NFT</div>
            <div className='px-4 font-semibold text-sm text-pink-400 hover:text-purple-500 cursor-pointer' onClick={()=> setTab(3)} >UPDATE NFT</div>
        </div>
        <div className='Right mr-2 flex items-center'>
            <div className='mr-7 text-3xl cursor-pointer'>🦊</div>
